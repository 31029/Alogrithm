...
func lowestCommonAncestorRecursion(root *TreeNode, p, q int, res []*TreeNode) (bool, bool) {
	...
	if root.Val == p {
		proot = true
		_, qL := lowestCommonAncestorRecursion(root.Left, p, q, res)
		_, qR := lowestCommonAncestorRecursion(root.Right, p, q, res)
		qroot = qL || qR
		return proot, qroot
	} else if root.Val == q {
		qroot = true
		pL, _ := lowestCommonAncestorRecursion(root.Left, p, q, res)
		pR, _ := lowestCommonAncestorRecursion(root.Right, p, q, res)
		proot = pL || pR
		return proot, qroot
	} else {
